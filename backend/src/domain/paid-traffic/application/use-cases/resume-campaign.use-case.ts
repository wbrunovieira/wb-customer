...
export type ResumeCampaignResult = Either<Error, { campaignId: string }>
...
export class ResumeCampaignUseCase {
  ...
  async execute(req: ResumeCampaignRequest): Promise<ResumeCampaignResult> {
    ...
    await this.adapter.resumeCampaign(adAccount.adAccountId, campaign.metaCampaignId)
